...
func (a *API) SendInvoice(chatID int64, title, description, payload, currency string, prices []LabeledPrice, opts *InvoiceOptions) (res ResponseMessage, err error) {
	...
	return res, a.get("sendInvoice", query.AddValues(vals, opts), &res)
}

// CreateInvoiceLink creates a link for an invoice.
func (a *API) CreateInvoiceLink(title, description, payload, currency string, prices []LabeledPrice, opts *CreateInvoiceLinkOptions) (res ResponseBase, err error) {
	var vals = make(url.Values)
	...
	vals.Set("currency", currency)
	vals.Set("prices", string(p))
	return res, a.get("createInvoiceLink", query.AddValues(vals, opts), &res)
}

// AnswerShippingQuery is used to reply to shipping queries.
// If you sent an invoice requesting a shipping address and the parameter is_flexible was specified,
// the Bot API will send an Update with a shipping_query field to the bot.
func (a *API) AnswerShippingQuery(shippingQueryID string, ok bool, opts *ShippingQueryOptions) (res ResponseBase, err error) {
	var vals = make(url.Values)

	vals.Set("shipping_query_id", shippingQueryID)
	vals.Set("ok", btoa(ok))
	return res, a.get("answerShippingQuery", query.AddValues(vals, opts), &res)
}
...
func (a *API) AnswerPreCheckoutQuery(preCheckoutQueryID string, ok bool, opts *PreCheckoutOptions) (res ResponseBase, err error) {
	var vals = make(url.Values)

	vals.Set("pre_checkout_query_id", preCheckoutQueryID)
	vals.Set("ok", btoa(ok))
	return res, a.get("answerPreCheckoutQuery", query.AddValues(vals, opts), &res)
}

// GetStarTransactions returns the bot's Telegram Star transactions in chronological order.
func (a *API) GetStarTransactions(opts *StarTransactionsOptions) (res ResponseStarTransactions, err error) {
	return res, a.get("getStarTransactions", query.UrlValues(opts), &res)
}

// RefundStarPayment refunds a successful payment in Telegram Stars.
func (a *API) RefundStarPayment(userID int64, telegramPaymentChargeID string) (res ResponseBool, err error) {
	var vals = make(url.Values)

	vals.Set("user_id", itoa(userID))
	vals.Set("telegram_payment_charge_id", telegramPaymentChargeID)
	return res, a.get("refundStarPayment", vals, &res)
}

// EditUserStarSubscription allows the bot to cancel or re-enable extension of a subscription paid in Telegram Stars.
func (a *API) EditUserStarSubscription(userID int64, telegramPaymentChargeID string, isCanceled bool) (res ResponseBool, err error) {
	var vals = make(url.Values)

	vals.Set("user_id", itoa(userID))
	vals.Set("telegram_payment_charge_id", telegramPaymentChargeID)
	vals.Set("is_canceled", btoa(isCanceled))
	return res, a.get("editUserStarSubscription", vals, &res)
}
