...
func (a *API) AnswerInlineQuery(inlineQueryID string, results []InlineQueryResult, opts *InlineQueryOptions) (res ResponseBase, err error) {
	var vals = make(url.Values)

	jsn, _ := json.Marshal(results)
	vals.Set("inline_query_id", inlineQueryID)
	vals.Set("results", string(jsn))
	return res, a.get("answerInlineQuery", query.AddValues(vals, opts), &res)
}

// SavePreparedInlineMessage stores a message that can be sent by a user of a Mini App.
func (a *API) SavePreparedInlineMessage(userID int64, result InlineQueryResult, opts *PreparedInlineMessageOptions) (res ResponsePreparedInlineMessage, err error) {
	var vals = make(url.Values)

	jsn, _ := json.Marshal(result)
	vals.Set("user_id", itoa(userID))
	vals.Set("result", string(jsn))
	return res, a.get("savePreparedInlineMessage", query.AddValues(vals, opts), &res)
}
